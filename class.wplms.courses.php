...
/**
 * Rule class responsible for Course pages protection.
 *
 * @category Membership
 * @package Model
 * @subpackage Rule
 * @subpackage WPLMS
 */
class Membership_Model_Rule_WPLMS_Courses extends Membership_Model_Rule {

	var $name = 'wplms_courses';
	var $label = 'Courses';
	var $description = 'Allows specific WPLMS Courses to be protected.';
	...
	function get_pages() {
		...
		wp_reset_postdata();
		return apply_filters( 'wplms_membership_courses', $course_pages );
	}
	...
	function admin_main($data) {
		...
		?>
		<div class='level-operation' id='main-wplms_courses'>
			<h2 class='sidebar-name'><?php _e('Courses', 'wplms-membership');?><span><a href='#remove' id='remove-wplms_courses' class='removelink' title='<?php _e("Remove WPLMS Course Pages from this rules area.",'wplms-membership'); ?>'><?php _e('Remove','wplms-membership'); ?></a></span></h2>
					...
					if($directory_pages) {
						...
						<table cellspacing="0" class="widefat fixed">
							...
							<tr>
								...
								<th style="" class="manage-column column-name" id="name" scope="col"><?php _e('Course title', 'wplms-membership'); ?></th>
								</tr>
							</thead>
						...
						foreach($directory_pages as $key => $page) { ?>
							...
							<tr valign="middle" class="alternate" id="post-<?php echo $post->ID; ?>">
								<th class="check-column" scope="row">
									<input type="checkbox" value="<?php echo $key; ?>" name="wplms_courses[]" <?php if(in_array($key, $data)) echo 'checked="checked"'; ?>>
								</th>
	...
	function add_viewable_posts( $wp_query ) {
		if ( !$wp_query->is_singular && empty( $wp_query->query_vars['pagename'] ) && ( !isset( $wp_query->query_vars['post_type'] ) || in_array( $wp_query->query_vars['post_type'], array( 'course', '' ) )) ) {
	...
	function validate_negative() {
		...
		return is_a( $page, 'WP_Post' ) && $page->post_type == 'course'
